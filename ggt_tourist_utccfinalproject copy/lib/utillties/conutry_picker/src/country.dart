...
class Country {
  ...
  bool startsWith(String query, CountryLocalizations? localizations) {
    ...
    if (query.startsWith("+")) {
      queryStr = query.replaceAll("+", "").trim();
    }
    return phoneCode.startsWith(queryStr.toLowerCase()) ||
        name.toLowerCase().startsWith(queryStr.toLowerCase()) ||
        countryCode.toLowerCase().startsWith(queryStr.toLowerCase()) ||
        (localizations
                ?.countryName(countryCode: countryCode)
                ?.toLowerCase()
                .startsWith(queryStr.toLowerCase()) ??
            false);
  }
