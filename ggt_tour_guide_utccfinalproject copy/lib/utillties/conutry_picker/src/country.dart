...
class Country {
  ...
  bool startsWith(String query, CountryLocalizations? localizations) {
    ...
    if (query.startsWith("+")) {
      strQuery = query.replaceAll("+", "").trim();
    }
    return phoneCode.startsWith(strQuery.toLowerCase()) ||
        name.toLowerCase().startsWith(strQuery.toLowerCase()) ||
        countryCode.toLowerCase().startsWith(strQuery.toLowerCase()) ||
        (localizations
                ?.countryName(countryCode: countryCode)
                ?.toLowerCase()
